...
def main():
    ...
    bddl_path = Path(bddl_file_name)
    bddl_parts = bddl_path.parts
    try:
        bddl_root_index = bddl_parts.index("bddl_files")
        task_relative_path = Path(*bddl_parts[bddl_root_index + 1 :])
    except ValueError:
        task_relative_path = Path(bddl_path.name)
    hdf5_path = (
        Path(get_libero_path("datasets"))
        / task_relative_path.parent
        / f"{task_relative_path.stem}_demo.hdf5"
    )

    output_parent_dir = hdf5_path.parent
    output_parent_dir.mkdir(parents=True, exist_ok=True)

    h5py_f = h5py.File(os.fspath(hdf5_path), "w")
